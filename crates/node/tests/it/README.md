...
1. **`TEMPO_BLOCK_HASH` storage slot**: Set to `keccak256(rlp(l1_header))`.
2. **`TEMPO_BLOCK_NUMBER` storage slot**: Set to `l1_header.number`.

The slot keys are generated by the native `TempoState` precompile storage
layout and referenced through `zone_precompiles::tempo_state::slots`.
