...
impl L1Subscriber {
            ...
            .map_ok(move |header| {
                ...
                async move {
                    ...
                    let (receipts, portal_logs) = tokio::try_join!(
                        ...
                        Self::get_portal_logs(provider, &l1_rpc_url, portal_address, block_number,),
                    )
                    .inspect_err(|_| {
                    ...
                    debug!(
                        ...
                        "Fetched and validated live block receipts and portal logs"
                    );
    ...
    async fn backfill(
        ...
        while let Some((header, receipts, portal_logs)) = fetched.try_next().await? {
            ...
            if events.is_empty() {
                let fallback = self.extract_portal_events_from_logs(block_number, &portal_logs);
                if !fallback.is_empty() {
                    self.record_portal_event_metrics(&fallback);
                    events = fallback;
                }
            }
    ...
    fn extract_portal_events_from_logs(&self, block_number: u64, logs: &[Log]) -> L1PortalEvents {
        ...
        for log in logs {
            if let Err(error) = events.push_log(log, block_number) {
                warn!(block_number, %error, "Failed to decode portal event from eth_getLogs");
            }
        }
        events
    }
