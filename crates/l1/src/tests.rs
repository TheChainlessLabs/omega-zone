...
fn test_push_log_decodes_legacy_deposit_made() {
    ...
    events.push_log(&log, 123).unwrap();

    let L1Deposit::Regular(deposit) = &events.deposits[0] else {
        panic!("legacy DepositMade should map to a regular deposit");
    };
    assert_eq!(deposit.token, token);
    assert_eq!(deposit.sender, sender);
    assert_eq!(deposit.to, recipient);
    assert_eq!(deposit.amount, 123_456);
    assert_eq!(deposit.fee, 789);
    assert_eq!(deposit.bounceback_recipient, sender);
}
