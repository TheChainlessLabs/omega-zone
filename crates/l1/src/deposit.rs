...
impl Deposit {
    ...
    /// Create a deposit emitted by portals that predate deposit numbers and
    /// explicit bounce-back recipients.
    pub(crate) fn from_legacy_event(event: legacy_portal_abi::DepositMade) -> Self {
        Self {
            token: event.token,
            sender: event.sender,
            to: event.to,
            amount: event.netAmount,
            fee: event.fee,
            bounceback_recipient: event.sender,
            memo: event.memo,
        }
    }
